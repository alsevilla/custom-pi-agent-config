...
- **Global scope**: `$HOME/.pi/agent/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `$HOME/.pi/agent/notes/`)
...
2. Glob `$HOME/.pi/agent/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `$HOME/.pi/agent/notes/` — works in any directory
