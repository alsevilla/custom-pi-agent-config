...
ones). Reference: @$HOME/.pi/agent/gsd-core/references/edge-probe.md.
...
# — NOT the consuming project's git root — falling back to git toplevel / $HOME/.pi/agent.
...
EDGE_PROBE_JS=$(for _c in \
  ...
  "$HOME/.pi/agent/gsd-core/bin/lib/edge-probe.cjs" \
  "$HOME/.pi/agent/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)
...
requirement list). Reference: @$HOME/.pi/agent/gsd-core/references/prohibition-probe.md — the
...
Use the SPEC.md template from @$HOME/.pi/agent/gsd-core/templates/spec.md.
