...
```markdown
Read $HOME/.pi/agent/agents/gsd-ui-researcher.md for instructions.
...
Template: $HOME/.pi/agent/gsd-core/templates/UI-SPEC.md
</output>
...
```markdown
Read $HOME/.pi/agent/agents/gsd-ui-checker.md for instructions.
...
@$HOME/.pi/agent/gsd-core/references/ui-consideration-probe.md.
...
# (#448) — NOT the consuming project's git root — falling back to git toplevel / $HOME/.pi/agent.
...
UI_PROBE_JS=$(for _c in \
  ...
  "$HOME/.pi/agent/gsd-core/bin/lib/ui-consideration-probe.cjs" \
  "$HOME/.pi/agent/bin/lib/ui-consideration-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)
